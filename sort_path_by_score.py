...
# 최소 시간 순
def sort_time(total_paths, drf_paths):
    ...
    time_sorted = np.sort(times)
    time_sorted_idx = np.argsort(times)
    paths_sorted = [paths[i] for i in time_sorted_idx]

    ''' drf 전달 데이터 빠른 시간 순으로 정렬 '''
    drf_sorted = [drf_paths[i] for i in time_sorted_idx]
...
# 최소 도보 순
def sort_walk(total_paths, drf_paths):
    ...
    walk_sorted = np.sort(walks)
    walk_sorted_idx = np.argsort(walks)
    paths_sorted = [paths[i] for i in walk_sorted_idx]

    ''' drf 전달 데이터 최소 도보 순으로 정렬 '''
    drf_sorted = [drf_paths[i] for i in walk_sorted_idx]
